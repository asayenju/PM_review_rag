...
def test_classify_query_allows_review_feedback_questions():
    ...
    assert classify_query("Give me an overview of the feature") == REVIEW_FEEDBACK
    assert classify_query("Summarize what customers are saying") == REVIEW_FEEDBACK
    assert classify_query("What's the general feeling about checkout?") == REVIEW_FEEDBACK
    assert classify_query("Tell me more") == REVIEW_FEEDBACK
...
def test_classify_query_allows_rating_review_questions():
    ...
    assert classify_query("What is the worst review?") == REVIEW_RATING
...
def test_classify_query_blocks_unrelated_questions():
    ...
    assert classify_query("What is the best movie?") == OUT_OF_SCOPE
    assert classify_query("Did bitcoin go up today?") == OUT_OF_SCOPE
    assert classify_query("What was the sports score?") == OUT_OF_SCOPE
